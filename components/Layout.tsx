  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] flex flex-col md:flex-row font-sans overflow-x-hidden">
      ...
      `}>
        <div className="p-8 h-full flex flex-col">
          <div className="hidden md:flex items-center gap-3 mb-10">
            <div className="w-10 h-10 bg-gradient-to-br from-indigo-600 to-violet-700 rounded-xl flex items-center justify-center shadow-lg">
          ...
          <button onClick={() => setIsProfileModalOpen(true)} className="mb-8 p-1 bg-slate-50 rounded-2xl border border-slate-100 w-full hover:border-indigo-200 hover:shadow-md transition-all group text-left relative">
             <div className="flex items-center gap-3 p-3">
                ...
                <div className="overflow-hidden flex-1">
                    ...
                    <p className="text-xs text-slate-500 capitalize">{role === 'trainer' ? 'Personal Trainer' : 'Aluno'}</p>
                </div>
             </div>
          </button>
          ...
          <button onClick={onLogout} className="flex items-center gap-3 px-4 py-3 text-slate-400 hover:bg-red-50 hover:text-red-600 rounded-xl transition-colors mt-auto font-medium text-sm">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto bg-[#F1F5F9] relative w-full">
        ...
        <div className="max-w-7xl mx-auto relative z-10 pb-20 md:pb-0">{children}</div>
      </main>

      {isProfileModalOpen && (
          <div className="fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 relative">
                  <button onClick={() => setIsProfileModalOpen(false)} className="absolute top-6 right-6 text-slate-400"><X className="w-6 h-6" /></button>
                  ...
                  <div className="flex flex-col items-center gap-6">
                      ...
                      <div className="w-full space-y-4">
                          <div><label className="block text-xs font-bold text-slate-400 uppercase mb-1">Nome Completo</label><input className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-indigo-500 font-medium" value={editName} onChange={(e) => setEditName(e.target.value)} /></div>
                          <button onClick={handleSaveProfile} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold shadow-lg flex items-center justify-center gap-2">Salvar Alterações</button>
                      </div>
                  </div>
              </div>
          </div>
