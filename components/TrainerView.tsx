...
export const TrainerViewContent: React.FC<TrainerViewProps> = ({ user, activeTab, onTabChange }) => {
        ...
        return (
            <div className="space-y-8 animate-fade-in pb-20">
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => { setEditingEventId(null); setNewEvent({ type: 'training', date: new Date().toISOString().split('T')[0], time: '08:00' }); setShowEventModal(true); }} className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black flex items-center gap-2 shadow-xl hover:scale-105 transition-all">
                                ...
                                <div key={e.id} className="flex items-center gap-6 p-6 bg-slate-50 rounded-3xl border border-transparent hover:border-indigo-100 hover:bg-white transition-all group relative">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-3 mt-1">
                                            ...
                                            <span className="flex items-center gap-1 text-xs text-indigo-600 font-black uppercase tracking-widest">
                                                <Users className="w-3 h-3" /> {e.attendees?.length || 0} Confirmados
                                            </span>
                                        </div>
                                    </div>
                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => handleEditEvent(e)} className="p-3 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all">
                                            <Edit3 className="w-5 h-5" />
                                        </button>
                                        <button onClick={() => handleDeleteEvent(e.id)} className="p-3 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all">
                ...
                {showEventModal && (
                    <div className="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
                        <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl p-8 animate-slide-up">
                            <h3 className="text-2xl font-black mb-6">{editingEventId ? 'Editar Evento' : 'Agendar Compromisso'}</h3>
                            <div className="space-y-4">
                                ...
                                <div className="flex gap-3 mt-6">
                                    ...
                                    <button onClick={handleAddEvent} className="flex-1 bg-slate-900 text-white py-4 rounded-xl font-black shadow-xl">
        ...
        return (
            <div className="flex gap-8 h-[calc(100vh-200px)] animate-fade-in">
                <div className="w-80 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col overflow-hidden">
                    <div className="p-6 border-b border-slate-50"><div className="relative"><Search className="absolute left-3 top-3 w-4 h-4 text-slate-400" /><input className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-sm outline-none" placeholder="Buscar aluno..." /></div></div>
                ...
                <div className="flex-1 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col overflow-hidden relative">
                    {activeChat ? (
                        <>
                            <div className="p-6 border-b border-slate-50 flex items-center gap-4 bg-slate-50/50"><div className="w-12 h-12 rounded-2xl bg-indigo-600 text-white flex items-center justify-center font-black text-xl shadow-md">{activeChat.name.charAt(0)}</div><div><h4 className="font-black text-slate-900">{activeChat.name}</h4><span className="text-[10px] text-green-500 font-black uppercase flex items-center gap-1"><span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span> Ativo agora</span></div></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {students.map(s => (
                        <button key={s.id} onClick={() => handleSelectStudent(s)} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all text-left group">
                            <div className="flex items-center gap-4 mb-6"><div className="w-16 h-16 rounded-3xl bg-slate-100 flex items-center justify-center text-2xl font-black text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all">{s.name.charAt(0)}</div><div><h3 className="font-bold text-lg text-slate-900">{s.name}</h3><p className="text-xs text-slate-500 font-medium">{s.email}</p></div></div>
                        </button>
                    ))}
                </div>
            </div>
    ...
    if (selectedStudent && builderState === 'manage') {
        return (
            <div className="space-y-8 animate-fade-in pb-20">
                <button onClick={() => setBuilderState('list')} className="flex items-center gap-2 text-slate-400 font-bold hover:text-slate-900 transition-colors"><ArrowLeft className="w-5 h-5" /> Voltar</button>
                <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col lg:flex-row gap-10">
                    <div className="w-32 h-32 rounded-[2.5rem] bg-indigo-600 flex items-center justify-center text-4xl font-black text-white shadow-2xl mb-4 relative overflow-hidden">{selectedStudent.name.charAt(0)}</div>
                    <div className="flex-1 space-y-6">
                        <h2 className="text-3xl font-black text-slate-900">{selectedStudent.name}</h2>
                        <div className="flex flex-wrap gap-3 pt-4"><ModuleToggle label="Fitness" active={selectedProfile?.activeModules.fitness} onClick={() => toggleModule('fitness')} icon={Dumbbell} loading={isActionLoading} /><ModuleToggle label="Espiritual" active={selectedProfile?.activeModules.spiritual} onClick={() => toggleModule('spiritual')} icon={Sparkles} loading={isActionLoading} /></div>
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6"><ToolCard icon={CheckSquare} title="Prescrever Treino" desc="Sincronize exercícios." onClick={() => {}} /><ToolCard icon={ImageIcon} title="Evolução Visual" desc="Fotos e timelapse." onClick={() => setBuilderState('view_progress')} /></div>
            </div>
        );
    }
    ...
    if (selectedStudent && builderState === 'view_progress') {
        ...
        return (
            <div className="space-y-8 animate-fade-in pb-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{progress.map(log => (<div key={log.id} className="bg-white rounded-[2rem] overflow-hidden shadow-sm border border-slate-100"><div className="aspect-square bg-slate-100">{log.photoUrl && <img src={log.photoUrl} className="w-full h-full object-cover" />}</div><div className="p-6"><p className="font-bold text-lg text-slate-900">{log.weight} kg</p></div></div>))}</div>
            </div>
        );
    }
