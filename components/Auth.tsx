...
import { Key, ArrowRight, User as UserIcon, Lock, Mail, Activity, Sparkles, ShieldAlert, Loader2 } from 'lucide-react';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isLogin) {
                ...
                if (firebaseUser) {
                    // Busca dados adicionais do usuário no Realtime Database
                    const userData = await db.getUserFromDb(firebaseUser.uid);
                    if (userData) {
                        if (userData.role !== roleToUse) {
                            throw new Error(`Esta conta não está registrada como ${isTrainerMode ? 'Personal' : 'Aluno'}.`);
                        }
                        onLogin(userData);
                    } else {
                        // Caso o usuário exista no Auth mas não no DB (raro)
                        const fallbackUser: User = { id: firebaseUser.uid, name: firebaseUser.displayName || 'Usuário', email: firebaseUser.email!, role: roleToUse };
                ...
                const userCredential = await cadastrarUsuario(email, password, name);
                const newUser: User = {
                    id: userCredential.user.uid,
                    name,
                    email,
                    role: roleToUse,
                    password: '' // Não salvamos a senha em texto plano no DB
                };
                
                // Salva no Database para persistência real
            ...
            <div className="w-full max-w-[1000px] grid grid-cols-1 md:grid-cols-2 bg-white rounded-[2rem] shadow-2xl overflow-hidden relative z-10 min-h-[600px] animate-fade-in">
                ...
                <div className={`p-10 md:p-12 flex flex-col justify-center ${isTrainerMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}`}>
                    <div className="mb-8">
                        ...
                        <p className="text-sm opacity-60">Utilize seu e-mail e senha cadastrados.</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        required
                                        type="text" 
                                        className={`w-full pl-12 pr-4 py-3 rounded-xl border-2 outline-none transition-all ${isTrainerMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-100'}`}
                                        value={name}
                        ...
                        {isTrainerMode && (
                            <div className="space-y-1">
                                ...
                                <input 
                                    required
                                    type="password" 
                                    className="w-full px-4 py-3 rounded-xl border-2 outline-none bg-slate-800 border-indigo-500/30 text-white"
                                    value={accessCode}
                        ...
                        <button 
                            ...
                            className={`w-full py-4 rounded-xl font-black text-lg flex items-center justify-center gap-2 transition-all shadow-xl ${
                                isTrainerMode ? 'bg-indigo-600 text-white' : 'bg-slate-900 text-white'
                            } disabled:opacity-50`}
                        >
                    ...
                    <div className="mt-8 text-center">
                        <button 
                            ...
                            className="text-sm font-bold opacity-60 hover:opacity-100 hover:underline"
                        >
