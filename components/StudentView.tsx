...
const ProgressView = ({ progress, user, onUpdate }: { progress: ProgressLog[], user: User, onUpdate: () => void }) => {
    ...
    // Estado do formulário de evolução
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                setNewLog(prev => ({ ...prev, photoUrl: reader.result as string }));
            };
    ...
    const handleSaveEvolution = async () => {
        if (!newLog.weight) {
            alert('Por favor, informe seu peso atual.');
            return;
        }
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h2 className="text-4xl font-black text-slate-900 tracking-tight">Evolução Timelapse</h2>
                    <p className="text-slate-500 font-medium">Sua transformação visual e física em um só lugar.</p>
                </div>
                <button 
                    onClick={() => setShowModal(true)}
                    className="w-full md:w-auto bg-slate-900 text-white px-8 py-5 rounded-[1.5rem] font-black shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3"
                >
                    <Plus className="w-5 h-5 text-indigo-400" /> Novo Registro Hoje
                </button>
            </div>

            {progress.length === 0 ? (
                <div className="bg-white p-20 rounded-[3rem] text-center border-2 border-dashed border-slate-100 flex flex-col items-center justify-center gap-6">
                    <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center text-slate-200">
                        <Camera className="w-10 h-10" />
                    </div>
                    <div className="space-y-2">
                        <p className="text-slate-400 font-black text-xl">Nenhuma evolução registrada.</p>
                        <p className="text-slate-300 font-medium">Capture sua primeira foto de progresso agora!</p>
                    </div>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {progress.map((log) => (
                        <div key={log.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-100 group hover:shadow-xl transition-all">
                            <div className="aspect-[3/4] bg-slate-100 relative overflow-hidden">
                                {log.photoUrl ? (
                                    ...
                                    <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 gap-2">
                                        ...
                                        <span className="text-[10px] font-black uppercase">Sem foto registrada</span>
                                    </div>
                                )}
                                <div className="absolute top-6 left-6 bg-slate-900/80 backdrop-blur-md text-white px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest">
                            ...
                            <div className="p-8 space-y-6">
                                <div className="flex justify-between items-center">
                                    <div>
                                        <p className="font-black text-3xl text-slate-900 tracking-tight">{log.weight} <span className="text-sm text-slate-400">kg</span></p>
                                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">Massa Corporal</p>
                                    </div>
                                ...
                                {log.measurements && (
                                    <div className="grid grid-cols-2 gap-3 pt-4 border-t border-slate-50">
                                        <MedidaDisplay label="Cintura" value={log.measurements.waist} />
                                        <MedidaDisplay label="Quadril" value={log.measurements.hips} />
                                        <MedidaDisplay label="Braço" value={log.measurements.arm} />
                                        <MedidaDisplay label="Perna" value={log.measurements.leg} />
                                    </div>
                                )}

                                {log.notes && (
                                    <p className="text-xs text-slate-500 font-medium italic bg-slate-50 p-4 rounded-xl">
                                        "{log.notes}"
                                    </p>
                                )}
                            </div>
            ...
            {/* Modal de Registro de Evolução */}
            {showModal && (
                <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6 transition-all animate-fade-in">
                    <div className="bg-white w-full max-w-4xl rounded-[3rem] p-10 shadow-2xl space-y-8 slide-up overflow-y-auto max-h-[90vh] custom-scrollbar">
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white">
                                    <Camera className="w-6 h-6" />
                                </div>
                                <h2 className="text-3xl font-black text-slate-900 tracking-tight">Registrar Evolução</h2>
                            </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                            ...
                            <div className="space-y-4">
                                <label className="block text-center cursor-pointer group">
                                    <div className={`aspect-[3/4] rounded-[2.5rem] border-4 border-dashed transition-all overflow-hidden flex flex-col items-center justify-center gap-4 ${newLog.photoUrl ? 'border-indigo-500 bg-indigo-50' : 'border-slate-100 bg-slate-50 group-hover:bg-slate-100 group-hover:border-indigo-200'}`}>
                                        {newLog.photoUrl ? (
                                            ...
                                            <>
                                                ...
                                                <div className="text-center">
                                                    <p className="text-sm font-black text-slate-500 uppercase">Escolher Foto</p>
                                                    <p className="text-[10px] font-bold text-slate-300 uppercase mt-1">Galeria ou Câmera</p>
                                                </div>
                                            </>
                                        )}
                                    </div>
                                ...
                                {newLog.photoUrl && (
                                    <button 
                                        onClick={() => setNewLog(prev => ({ ...prev, photoUrl: '' }))}
                            ...
                            <div className="space-y-8">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Peso Atual (Kg)</label>
                                    <input 
                                        type="number" 
                                        className="w-full p-6 bg-slate-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl outline-none font-black text-3xl transition-all" 
                                        placeholder="0.0" 
                                        value={newLog.weight}
                                ...
                                <div className="space-y-4">
                                    <h3 className="text-xs font-black uppercase text-slate-900 flex items-center gap-2">
                                        <Ruler className="w-4 h-4 text-indigo-600" /> Medidas de Controle (CM)
                                    </h3>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-slate-400 ml-1 tracking-widest">Observações</label>
                                    <textarea 
                                        className="w-full p-6 bg-slate-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl outline-none font-medium text-sm transition-all h-32 resize-none" 
                                        placeholder="Como está o seu progresso? Alguma dificuldade?"
                                        value={newLog.notes}
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 pt-6">
                            <button 
                                ...
                                className="flex-1 py-5 rounded-[1.5rem] bg-slate-100 text-slate-500 font-black text-lg hover:bg-slate-200 transition-all"
                            >
                                Cancelar
                            </button>
                            <button 
                                ...
                                disabled={isSaving}
                                className="flex-[2] bg-slate-900 text-white py-5 rounded-[1.5rem] font-black text-lg shadow-2xl flex items-center justify-center gap-3 hover:bg-slate-800 transition-all disabled:opacity-50"
                            >
                                {isSaving ? <Loader2 className="w-6 h-6 animate-spin" /> : <><Save className="w-6 h-6 text-indigo-400" /> Salvar Evolução</>}
                            </button>
                        </div>
                    </div>
                </div>
...
const MedidaInput = ({ label, value, onChange }: { label: string, value: string, onChange: (v: string) => void }) => (
    <div className="space-y-1">
        ...
        <input 
            type="number" 
            className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-xl outline-none font-bold text-sm transition-all" 
            placeholder="0.0"
            value={value}
